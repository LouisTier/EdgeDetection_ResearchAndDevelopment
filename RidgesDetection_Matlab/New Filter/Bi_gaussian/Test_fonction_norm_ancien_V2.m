...
abscisse = -30:1:30; %Plage de la gaussienne

rho = 1; %Scale ratio
sigma = [0.58 1.81 2.88 3.91 4.93]% 5.94 6.95 7.95 8.96]; %Paramètre de la gaussienne centrale

%sigma = [0.58 2 3 4 5 6 7 8 9]; %Paramètre de la gaussienne centrale
...
%     for i = 1:1:nbRidge     % build signal
%         R = 2*i-1;
%         s = [s ones(1,R) zeros(1, 3)];
%     end

    s = [s ones(1,9) zeros(1,D) ones(1,3) zeros(1,D-2) ones(1,1) zeros(1,D-1) ones(1,5) zeros(1,D) ones(1,7) zeros(1,D+10)];
...
%     subplot(2,1,2),fplot(BG2)
    ...
    figure(2), %title(['rho = ',num2str(rho)],'FontSize', 11, 'FontName', 'times')
    subplot(nbRidge,1, i), hold on,
    %     subplot(nbRidge,2, 2*i), hold on,
    bar (s), plot(s2(i,:), 'linewidth', 20), %title(['convolution with G^{(2)}_\sigma, \sigma = ',num2str(sigma(i))], 'FontSize', 11, 'FontName', 'times')
    h_x = xlabel(['\fontname{times} \sigma = ',num2str(sigma(i))])
    set(h_x, 'FontSize', 30) 
    set(h_x,'FontWeight','bold') %bold font
    %     ylim([0 1.6])
    xlim([15 length(s)-10])
    [s2_max(i), index(i)] = max(s2(i,:));
    plot( index(i), s2_max(i), 'ko', 'linewidth', 20 )
    xticks([]),
    exportgraphics(gcf,'reponse_rho_1.pdf','ContentType','vector')
    %saveName = (['rho = ', num2str(rho)]);
