...
% calcule la carte de probabilité de blobness (tubularité locale) d'une image d'entrée 2D
%  
% blobness = blobness2D(I, sigmas, spacing, tau, brightondark)
%  
% entrées :
...
% 2) sigmas : vecteur d'échelles sur lequel le blobness est calculé

% 3) espacement : résolution de l'espacement de l'image d'entrée - pendant le calcul de la matrice hessienne, 
% la taille du noyau du filtre gaussien dans chaque dimension peut être ajustée pour tenir compte de l'espacement 
% différent de l'image pour les différentes dimensions. 

% 4) tau : (entre 0,5 et 1) : paramètre qui contrôle l'uniformité de la réponse - plus tau est faible -> réponse de sortie 
% plus intense            
 
% 5) brightondark : (vrai/faux) : les blobs (structures sphériques) sont-ils clairs sur un fond sombre ou sombres sur un 
% fond clair (le défaut pour la 2D est faux).
 
% sorties : 
% blobness : réponse blobness maximale sur les échelles sigma
