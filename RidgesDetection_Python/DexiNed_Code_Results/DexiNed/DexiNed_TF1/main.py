...
#On n'utilisera pas cette partie
...
#Cette fonction nous permet de passer le modele en mode test
#Cela nous permettra de pouvoir le tester sur n'importe quelle image
def test(checkpoint_path, dataloader, model, device, output_dir, args):
    ...
    #Chargement du modele a partir du checkpoint telechargé sur le Github
    model.load_state_dict(torch.load(checkpoint_path,
                                     map_location=device))

    #On met le modele en mode test
    model.eval()

    #On va parcourrir toutes les images qui sont dans le fichier "test"
    with torch.no_grad():
        total_duration = []
        for batch_id, sample_batched in enumerate(dataloader):
            images = sample_batched['images'].to(device)
...
#Definition d'arguments pour lancer le code via l'interpreteur de commande
def parse_args():
...
def main(args):
    ...
    dataloader_val = DataLoader(dataset_val,
                                ...
                                shuffle=False,
                                num_workers=args.workers)
    #Cette partie du code nous est utile
    if args.is_testing:
